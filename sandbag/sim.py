...
class CBSSolver:
    ...
    def find_solution(self):
        ...
        for i in range(self.num_of_agents):
            ...
            if path is None:
                logger.warning(f"No path found for agent {i}")
                return None
            root['paths'].append(path[0])
        
        root['cost'] = get_sum_of_cost(root['paths'])
        root['collisions'] = detect_collisions(root['paths'], self.goals, root['conflict_freq'])
        self.push_node(root)
        
        while self.open_list:
            if self.should_terminate():
                logger.warning("CBS terminated due to resource limits")
                break
            
            node = self.pop_node()
            
            if not node['collisions']:
                logger.info(f"CBS found solution! Expanded: {self.num_of_expanded}, Generated: {self.num_of_generated}")
                return node['paths']
            ...
            for constraint in constraints:
                child = {
                    ...
                    'conflict_freq': dict(node['conflict_freq']),
                    'collisions': []
                }
                
                ai = constraint['agent']
                astar = A_Star(self.my_map, self.starts, self.goals, self.heuristics, ai, child['constraints'])
                path = astar.find_paths()
                
                if path is not None:
                    child['paths'][ai] = path[0]
                    child['collisions'] = detect_collisions(child['paths'], self.goals, child['conflict_freq'])
                    child['cost'] = get_sum_of_cost(child['paths'])
                    self.push_node(child)
        
        logger.warning("CBS could not find solution")
        return None
...
class Environment:
    ...
    def _plan_paths(self):
        ...
        self.cbs_paths = cbs_solver.find_solution()
        ...
        if self.cbs_paths:
            ...
            logger.warning("CBS failed to find paths, falling back to individual pathfinding")
            for agent in self.agents:
                agent.cbs_path = None
